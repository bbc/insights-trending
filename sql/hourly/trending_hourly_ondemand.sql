...
-- 645421 rows --
DROP TABLE IF EXISTS central_insights_sandbox.trending_hourly_ondemand_step1;
CREATE TABLE central_insights_sandbox.trending_hourly_ondemand_step1 AS
...
-- 3084941 rows --
DROP TABLE IF EXISTS central_insights_sandbox.trending_hourly_ondemand_step2;
CREATE TABLE central_insights_sandbox.trending_hourly_ondemand_step2 AS
SELECT
...
FROM prez.scv_vmb
WHERE brand_title IN (SELECT tleo FROM central_insights_sandbox.trending_hourly_ondemand_step1)
OR series_title IN (SELECT tleo FROM central_insights_sandbox.trending_hourly_ondemand_step1)
OR episode_title IN (SELECT tleo FROM central_insights_sandbox.trending_hourly_ondemand_step1)
OR presentation_title IN (SELECT tleo FROM central_insights_sandbox.trending_hourly_ondemand_step1)
OR clip_title IN (SELECT tleo FROM central_insights_sandbox.trending_hourly_ondemand_step1);

-- STEP 3: Pick most common genre for each TLEO --
-- 325785 rows --
-- 13.5 seconds --
DROP TABLE IF EXISTS central_insights_sandbox.trending_hourly_ondemand_step3;
CREATE TABLE central_insights_sandbox.trending_hourly_ondemand_step3 AS
...
FROM central_insights_sandbox.trending_hourly_ondemand_step1 AS a
LEFT JOIN central_insights_sandbox.trending_hourly_ondemand_step2 AS b ON a.tleo = b.tleo
...
-- 1109056 rows --
DROP TABLE IF EXISTS central_insights_sandbox.trending_hourly_ondemand_step4;
CREATE TABLE central_insights_sandbox.trending_hourly_ondemand_step4 AS

WITH b AS (
  SELECT DISTINCT destination, tleo, genre
  FROM central_insights_sandbox.trending_hourly_ondemand_step3
)

SELECT DISTINCT a.datehour, b.destination, b.tleo, b.genre
FROM central_insights_sandbox.trending_hourly_ondemand_step1 AS a, b;

-- STEP 5: Retrieve streams --
-- 1109056 rows --
DROP TABLE IF EXISTS central_insights_sandbox.trending_hourly_ondemand_step5;
CREATE TABLE central_insights_sandbox.trending_hourly_ondemand_step5 AS
...
FROM central_insights_sandbox.trending_hourly_ondemand_step4 AS a
LEFT JOIN central_insights_sandbox.trending_hourly_ondemand_step3 AS b
...
-- STEP 6 --
-- 1109056 rows --
DROP TABLE IF EXISTS central_insights_sandbox.trending_hourly_ondemand_step6;
CREATE TABLE central_insights_sandbox.trending_hourly_ondemand_step6 AS
...
FROM central_insights_sandbox.trending_hourly_ondemand_step5;
